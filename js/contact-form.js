...
(function($) {
    ...
    $("#contactForm").validator().on("submit", function(event) {
        ...
        if (event.isDefaultPrevented()) {
            formError();
            submitContactFormActionMSG(false, "Prosimy o poprawne wypełnienie formularza!");
        } else {
            var fname = $("#fname").val();
            var email = $("#email").val();
            var phone = $("#phone").val();
            ...
            if (validphone > 0) {
                ...
                submitContactFormActionMSG(false, "Proszę podać numer telefonu");
                return false;
            }
        }
    });
    ...
    function contactFormSuccess() {
        submitContactFormActionMSG(true, "Dane zostały pomyślnie wysłane!");
    }
